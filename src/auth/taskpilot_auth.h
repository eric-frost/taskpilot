...
/** Privileged helper run as root via polkit/D-Bus (KAuth). Each public slot is
    a polkit action declared in io.github.ericfrost.taskpilot.actions; arguments arrive in a
...
class TaskpilotAuthHelper : public QObject {
    Q_OBJECT
public Q_SLOTS:
    /** unmount a single target path (io.github.ericfrost.taskpilot.unmount). */
    ActionReply unmount(const QVariantMap &args);

    /** Listening TCP sockets with full process info, resolved as root so
        root-owned sockets aren't blank (io.github.ericfrost.taskpilot.ports). Returns
        rows under data key "rows" as a list of QStringList. */
    ActionReply ports(const QVariantMap &args);

    /** Run `bindln` as root with whitelisted args (io.github.ericfrost.taskpilot.bindln);
    ...
    /** List docker + root podman containers as root so they appear without
        docker-group membership (io.github.ericfrost.taskpilot.dockerlist). No client input;
        only ps/stats are run (never inspect). Rows under data key "rows". */
    ActionReply dockerlist(const QVariantMap &args);

    /** Read another user's crontab as root (io.github.ericfrost.taskpilot.cronread, session).
        args "cmd"="users" → data["users"]=[{user,count}]; "cmd"="list"+"user"
        → data["text"]. Read-only. */
    ActionReply cronread(const QVariantMap &args);

    /** Replace a user's crontab as root (io.github.ericfrost.taskpilot.cronsave; NOT session-
        cached — each write re-authenticates). args "user"+"text". */
    ActionReply cronsave(const QVariantMap &args);

    /** Send a signal to a process as root (io.github.ericfrost.taskpilot.kill). Args:
